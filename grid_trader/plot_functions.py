...
def plot_single_atr_grid(df, atr, atr_grid, closes_mean, hist):
    ...
    # %%
    kl_go = go.Candlestick(x=df['open_time'],
                    open=df['open'],
                    high=df['high'],
                    low=df['low'],
                    close=df['close'])
    ...
    atr_go = go.Scatter(x=df.open_time, y=atr,
                                mode="lines",
                                line=go.scatter.Line(color="gray"),
                                showlegend=False)
                                

    ema_go = go.Scatter(x=df.open_time, y=closes_mean,
                                mode="lines",
    ...
    hist_go = go.Scatter(x=df.open_time, y=hist,
                                mode="lines+markers",
                                # line=go.scatter.Line(color="blue"),
                                showlegend=False,
    ...
    def plot_atr_grid(atr_grid, fig):
        for atr_band in atr_grid:
            if sum(atr_band) >= 1.2 * sum(closes_mean):
                color = 'red'
            else:
                color = "teal"
            atr_go = go.Scatter(x=df.open_time, y=atr_band,
                                mode="lines",
